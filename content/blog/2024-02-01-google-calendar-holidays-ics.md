---
title: "Google Calendar holidays ICS"
date: 2024-02-01
issueId: 120
---

In another attempt to free myself from google apps, i replaced the calendar app in my phone with [Fossify Calendar](https://github.com/FossifyOrg/Calendar).

One of the problems i had is that i no longer have the holidays events.

After searching a little on the internet, i found the following posts that allows you to get the ics file from a country's holidays. [Google Calendar - Public Holiday Calendars](https://gist.github.com/dhoeric/76bd1c15168ee0ee61ad3bf1730dcb65) with the IDs of country holidays ids. [How to find the iCal address for a public Google calendar](https://grantwinney.com/how-to-find-the-ical-address-for-a-public-google-calendar/) to get the ICS file.

You need to url encode the calendar id, and you could change the `en.ar` to `es.ar` if you want them in spanish.

Here is [the url that i need for my country](https://calendar.google.com/calendar/ical/es.ar%23holiday%40group.v.calendar.google.com/public/basic.ics) for future reference:
```
https://calendar.google.com/calendar/ical/es.ar%23holiday%40group.v.calendar.google.com/public/basic.ics
```

---

Here is the csv file from the first post in case its unavailable in the future:
```csv
Religion/Country,calendarID
...
Orthodox Holidays,en.orthodox_christianity#holiday@group.v.calendar.google.com
...
And the url from the second post is `https://calendar.google.com/calendar/ical/{CALENDAR_ID}/public/basic.ics`
